...
from YaeRobot import OWNER_ID
from YaeRobot.events import register
...
async def lego(event):
 ...
 if event.sender_id != OWNER_ID and not quew:
  await event.reply('`Give Some Text To Create Logo​ !`\n`Example /logo <Saber>`')
  return
 ...
 try:
    ...
    fillcolor = "black"
    shadowcolor = "blue"
    fnt = glob.glob("./YaeRobot/LOGO_FONT/*")
    ...
    fname = "fallen.png"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Logo Created By @YaeRobot")         
    await pesan.delete()
    if os.path.exists(fname):
            os.remove(fname)
 except Exception as e:
    await event.reply(f'Floodwait Error, Report This At @NexusXSupport')


__mod_name__ = "Logo​"

__help__ = """
YaeRobot Forger can create some beautiful and attractive logo for your profile pics.
