...
from YaeRobot import (DEV_USERS, OWNER_ID, DRAGONS, SUPPORT_CHAT, DEMONS,
                          TIGERS, WOLVES, dispatcher)
from YaeRobot.modules.helper_funcs.chat_status import (dev_plus, sudo_plus,
                                                           whitelist_plus)
from YaeRobot.modules.helper_funcs.extraction import extract_user
from YaeRobot.modules.log_channel import gloggable
from telegram import ParseMode, TelegramError, Update
from telegram.ext import CallbackContext, CommandHandler, run_async
from telegram.utils.helpers import mention_html

ELEVATED_USERS_FILE = os.path.join(os.getcwd(),
                                   'YaeRobot/elevated_users.json')
...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("This member is already a Sword Master")
        return ""

    if user_id in DEMONS:
        rt += "Requested HA to promote a Sword Slasher Hunter to Sword Master."
        data['supports'].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        rt += "Requested HA to promote a Sword Holder to A Sword Master."
    ...
    update.effective_message.reply_text(
        rt + "\nSuccessfully set this nigga as sudo ass!".format(
...
Group admins/group owners do not need these commands. 
 ...
 ╠ `/aranks`*:* Lists all Sword Master
 ╠ `/branks`*:* Lists all Sword Slashers
 ╠ `/cranks`*:* Lists all Sword Disasters
 ╠ `/dranks`*:* Lists all Sword Holders
 ╠ `/sranks`*:* Lists all Sword Demons
 ╠ `/addarank`*:* Adds a user to Sword Master
 ╠ `/addbrank`*:* Adds a user to Sword Slashers
 ╠ `/addcrank`*:* Adds a user to Sword Disasters
 ╠ `/adddrank`*:* Adds a user to Sword Holders
